    ...
    <view style="height: 100%">
        <view class="container">
            <view class="header_box">
                <view class="home_header">
                    <view class="header_txt">
                        <view class="txt_desc">
                            <image class="img_rmb" src="/static/assets/image/home_rmb_icon.png"></image>
                            邀您快乐刷刷刷 无忧支付
                        </view>
            ...
            <view class="home_notice">
                <view class="messge">
                    <swiper :autoplay="true" :duration="500" :indicatorDots="false" :interval="5000" style="height: 120rpx; width: auto" :vertical="true">
                        <view v-if="messgeArr.length > 0">
                            <swiper-item v-for="(item, index) in messgeArr" :key="index">
                                <view class="messgeTitle">
                                    <view class="notice_read_stauts" v-if="item.isRead == 'unread'"></view>
                                    {{ item.title }}
                                </view>
            ...
            <view class="cust_box">
                ...
                <view class="shareBox" v-if="singShare">
                    ...
                    <view @tap="eventShareHint" class="yhqbox">
                        ...
                        <view class="rightImg" style="background-image: url(https://www.krppay.com/skb_assets/home_icon_share.png)">
                            <view class="desc">分享入口</view>
                        </view>
                        <button
                            openType="share"
                            style="width: 100%; height: 120rpx; position: absolute; top: 0; background-color: transparent"
                            v-if="bindFlag && chargeFlag"
                        ></button>
                    </view>
                </view>
                <view @tap="eventShareHint" class="singeshare" v-if="!singShare">
                    <image mode="aspectFill" src="https://www.krppay.com/skb_assets/home_icon_singeshare.png"></image>
                    <button openType="share" style="width: 100%; height: 120rpx; position: absolute; top: 0; background-color: transparent" v-if="bindFlag && chargeFlag"></button>
                </view>
                <scroll-view
                    @scroll="scroll"
                    ...
                    style="width: 100%"
                >
                    <view class="optionList" :style="'width: ' + (optionList2.length > 4 ? '200%' : '100%') + ';'">
                        <view @tap="clickOption" class="option" :data-item="item" :style="'width: ' + scrollWidth + 'px;'" v-for="(item, index) in optionList2" :key="index">
                ...
                <view class="page" v-if="optionList2.length > 4">
                    <image :src="optionFirstPage == true ? '/static/assets/image/home_icon_left.png' : '/static/assets/image/home_icon_right.png'"></image>
                </view>
            </view>
            <view class="hot_box">
                <view class="cust_title" style="margin-top: 70rpx">热门活动</view>
                <swiper :autoplay="true" class="index_banner_swiper" :indicatorDots="true" v-if="activityDataList.length > 0">
                    <swiper-item v-for="(bannerItem, index) in activityDataList" :key="index">
                        <van-image
                ...
                <view class="adimg" v-else>
                    <image src="https://www.krppay.com/skb_assets/home_ad_icon.png"></image>
                </view>
            </view>
            <view style="background-color: rgb(255, 238, 237); height: 40rpx; width: 100%"></view>
        </view>
        <van-popup customStyle="background-color: transparent;" :show="showNoticeImgDialog">
            <view class="pop_box">
                <image
                    @tap="closeImgPop"
                    mode="aspectFit"
                ...
                ></image>
                <image mode="widthFix" :src="alertNoticeBean.imagURl" style="padding: 20rpx; width: 600rpx"></image>
                <van-checkbox @change="onCheckImgPop" checkedColor="#222124" customClass="img_check" iconSize="38rpx" :value="notPopPrompt">不再提示</van-checkbox>
            </view>
        </van-popup>
        <van-popup customStyle="background-color: transparent;" :show="showNoticeContentDialog">
            <view class="pop_box">
                <view class="pop_content_box">
                    <image
                        ...
                        style="width: 60rpx; height: 60rpx; align-self: flex-end"
                    ></image>
                    <image mode="aspectFit" src="https://www.krppay.com/skb_assets/home_bgsystem_messge.png" style="align-items: center; width: 272rpx; height: 198rpx"></image>
                ...
                <van-checkbox @change="onCheckContentPop" checkedColor="#222124" customClass="img_check" iconSize="38rpx" :value="notPopContentPrompt">不再提示</van-checkbox>
            </view>
        </van-popup>
    </view>
</template>
...
export default {
    ...
    onShareAppMessage: function () {
        ...
        return {
            title: '收款呗邀请您开启便捷收款',
            path: '/login/accontRegister/accontRegister?inviterCode='.concat(this.custInfo.inviterCode),
    ...
    methods: {
        ...
        clickMassge: function () {
            ...
            uni.navigateTo({
                url: '../../notice/noticeList/noticeList'
            });
        },
            ...
            this.isLimitNextStep() ||
                uni.getBackgroundFetchToken({
                    complete: function (t) {
                        ...
                        var e = t.token;
                        uni.navigateTo({
                            url: '/toolsBox/webview/webview?url='.concat(encodeURIComponent(o + 'withdrawalFeeCoupon?token=' + e + '&source=WLP'))
                        });
                    }
                });
        },
        ...
        queryEntrance: function () {
            ...
            t.queryEntrance().then(function (t) {
                if (200 == t.code) {
                    that.updateListState('快速到账', 'TRUE' == t.object.debitCardT0Privilege);
                }
            });
        },
